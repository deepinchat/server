...
{
    Task<ContactDto?> CreateContactAsync(ContactRequest request, CancellationToken cancellationToken = default);
    ...
    Task<ContactDto?> UpdateContactAsync(Guid id, ContactRequest request, CancellationToken cancellationToken = default);
}
        ...
        : BaseClient(httpClient, options, logger), IContactApiClient
{
    ...
    public async Task<ContactDto?> CreateContactAsync(ContactRequest request, CancellationToken cancellationToken = default)
    {
        return await PostAsync<ContactDto?>("api/v1/contacts", request, cancellationToken);
    }

    public async Task<ContactDto?> UpdateContactAsync(Guid id, ContactRequest request, CancellationToken cancellationToken = default)
    {
